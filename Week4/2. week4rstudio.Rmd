...
```{r}
nicadMean <- mean(BattPop$Time[BattPop$Battery==1])
...
```{r}
replBattCI <- function() {
  mySamp <- BattPop[sample(100000, size=172, replace=TRUE), ]
...
# str(CIlist)
# View(CIlist)
l <- length(which(CIlist[,1] > popMeanDiff))
m <- length(which(CIlist[,2] < popMeanDiff))

cat("number of instances where red/green lines crossing horizontal line:",l+m)
